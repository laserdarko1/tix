...
class TicketCommandsCog(commands.Cog):
    ...
    @app_commands.command(name="ticketcount", description="Show ticket statistics")
    async def ticket_count(self, interaction: discord.Interaction):
        """Show ticket count statistics"""
        try:
            guild_id = interaction.guild.id
            
            # Get total active tickets
            active_total = await db.get_active_tickets_count(guild_id)
            
            # Get ticket statistics per category
            stats = await db.get_ticket_stats(guild_id)
            
            embed = discord.Embed(
                title="📊 Ticket Statistics",
                description="Current ticket count information",
                color=discord.Color.blue()
            )
            ...
            embed.add_field(
                name="🔥 Active Tickets",
                value=f"**{active_total}** tickets currently open",
                inline=False
            )
            ...
            if stats:
                ...
                for category, count in stats.items():
                    active_count = await db.get_active_tickets_count(guild_id, category)
                    stats_text += f"**{category}:** {count} total ({active_count} active)\n"
                    total_opened += count
                
                embed.add_field(
                    name="📈 Total Tickets Opened",
                    value=f"**{total_opened}** tickets opened all time",
                    inline=False
                )
                ...
                embed.add_field(
                    name="📈 Total Tickets Opened",
                    value="No tickets have been opened yet",
                    inline=False
                )
            
            embed.set_footer(text="Statistics are tracked per server")
            embed.timestamp = discord.utils.utcnow()
            
            await interaction.response.send_message(embed=embed, ephemeral=True)
            
        except Exception as e:
            await interaction.response.send_message(f"❌ Error getting ticket count: {str(e)}", ephemeral=True)
